...
Suppose we posit that $b_1 = \beta$. So far, we've described the distribution of $$t = \frac{\hat b_1 - \beta}{\sqrt{\widehat{\text{var}}(\hat b_1)}}$$ when the null hypothesis is true. But what about when the null hypothesis is false? Turns out this is a little bit harder, but we can simulate the distribution. Suppose we are interested in the null hypothesis where $\beta = 0$; i.e.,: $$H_0: b_1 = 0 \qquad  \text{vs} \qquad  H_A: b_1 \neq 0.$$
...
for(i in 1:sim.size){
  ...
  # Form the dependent variable Y where the null hypothesis is true
    ...
    rec[i, ] <- c(
      summary(reg.trueNull)$coeff[2, 3], 
      summary(reg.trueNull)$coeff[2, 4],
      summary(reg.falseNull)$coeff[2, 3], 
      summary(reg.falseNull)$coeff[2, 4]
      )
}
