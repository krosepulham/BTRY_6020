...
```{r}
#A question was asked in class: if we omit the interaction, will the coefficient estimate for the effect of age change? We can write some code to investigate this: 
model6 <- lm(log(price) ~ age + quality, data = housing_data)
summary(model6)

#We see that the coefficients are different. This is because in the model with the interaction, the coefficient of age is the slope for the "quality=high" group, and in the no-interaction model it is the slope for all three groups. 
```
